...
{
    class PageHandler:BaseHandler, IItemHandler
    {
        ...
        {   
            ...
            postNode.SetAttribute("propagate", input.Propagate.ToString());
            postNode.SetAttribute("type", "P");
            ...
            if(input.GetTextFromItem){
                string templateId = _settings["PageTemplate" + acc.Type];
                if (templateId == TcmUri.UriNull.ToString())
                {
                    templateId = data.PageTemplateId;
                }                
                text = RenderItem(data.PageId, templateId);
            }else{
                text = input.Text;
            }            
            ...
            if (input.GetLink) {
                linkURL = _settings["LinkURL"] + "?pageId=" + data.PageId;
            }
